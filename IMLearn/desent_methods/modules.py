...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        # X_w = X @ self.weights
        # return - np.sum(y * X_w - np.log(1 + np.exp(X_w))) / X.shape[0]
        return -np.sum(y * (X @ self.weights) - np.log(1 + np.exp(X @ self.weights))) / len(X)
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        # exp_Xw = np.exp(self.weights @ X.T)
        # return - (y @ X - X.T @ (exp_Xw / (1 + exp_Xw))) / X.shape[0]
        sigmoid_vals = np.exp((X @ self.weights)) / (1 + np.exp((X @ self.weights)))  # Compute sigmoid values
        return - (y @ X - X.T @ sigmoid_vals) / len(X)
