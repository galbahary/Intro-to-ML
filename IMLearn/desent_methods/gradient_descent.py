...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        curr_weights = f.weights
        weights_arr = [curr_weights]
        vals_arr = [f.compute_output(X=X, y=y)]

        for t in range(self.max_iter_):

            eta = self.learning_rate_.lr_step(t=t)
            grad = f.compute_jacobian(X=X, y=y)
            f.weights = curr_weights - eta * grad
            curr_val = f.compute_output(X=X, y=y)
            delta = np.linalg.norm(f.weights - curr_weights, ord=2)

            self.callback_(solver=self, weights=f.weights, val=curr_val, grad=grad, t=t, eta=eta, delta=delta)

            curr_weights = f.weights
            vals_arr.append(curr_val)
            weights_arr.append(curr_weights)
            if delta < self.tol_:
                break
        ...
        if self.out_type_ == OUTPUT_VECTOR_TYPE[1]:
            return weights_arr[np.argmin(vals_arr)]

        return np.mean(weights_arr, axis=1)
