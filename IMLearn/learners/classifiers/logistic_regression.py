...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        init_w = np.random.normal(0, 1, (X.shape[1])) / np.sqrt(X.shape[1])

        if self.penalty_ == "l1":
            module = RegularizedModule(LogisticModule(init_w), L1(init_w), self.lam_, init_w)
        elif self.penalty_ == "l2":
            module = RegularizedModule(LogisticModule(init_w), L2(init_w), self.lam_, init_w)
        else:
            module = LogisticModule(weights=init_w)
    ...
    def _predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        return self.alpha_ <= self.predict_proba(X)
