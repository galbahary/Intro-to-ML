...
class AdaBoost(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        """
        self.models_, self.weights_, self.D_ = [], np.zeros(self.iterations_),\
                                               np.ones(len(y), dtype=np.float64) / len(y)
        for i in range(self.iterations_):
            self.models_.append(self.wl_().fit(X, y * self.D_))
            pred = self.models_[i].predict(X)
            err = np.sum(self.D_[y != pred])
            self.weights_[i] = .5 * np.log(1. / err - 1)
            self.D_ = self.D_ * np.exp(-pred * y * self.weights_[i])
