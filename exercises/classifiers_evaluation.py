...
def run_perceptron():
    ...
    for n, f in [("Linearly Separable", "linearly_separable.npy"),
        ...
        def add_loss(perceptron, i, j):
            losses.append(perceptron.loss(X, y))

        perceptron = Perceptron(callback=add_loss)
        perceptron.fit(X, y)
        ...
        fig = go.Figure(data=go.Scatter(x=iterations, y=losses, mode="lines", marker=dict(color="green")),
                        layout=go.Layout(
                            title="Perceptron training error using " + n + " dataset",
                            xaxis_title="Iterations",
                            yaxis_title="Misclassification Error"))
        fig.write_image("perceptron_fit_" + n + ".png")
...
def compare_gaussian_classifiers():
    ...
    for f in ["gaussian1.npy", "gaussian2.npy"]:
        ...
        fig = make_subplots(rows=1, cols=2,
                            subplot_titles=(
                                "Gaussian Naive Bayes (accuracy= " + str(round(100 * accuracy(y, naive_pred), 2)) + ")",
                                "LDA (accuracy= " + str(round(100 * accuracy(y, lda_pred), )) + ")"))

        # Add traces for data-points setting symbols and colors
        fig.add_traces([go.Scatter(x=X[:, 0], y=X[:, 1], mode='markers',
                                   marker=dict(color=naive_pred, symbol=class_symbols[y], colorscale=class_colors(3))),
        ...
        fig.update_layout(title_text="Comparing Gaussian Classifiers - " + f[:-4] + " dataset",
                          width=800, height=400, showlegend=False)
        fig.write_image("lda.vs.naive.bayes." + f[:-4] + ".png")
        # fig.show()
